...
},_setOption:function(a,b){if(a==="page"){this._setPage(b)
}else{$.Widget.prototype._setOption.apply(this,arguments)
}},_setPage:function(c){var a=this.getPageCount();
if(c>=0&&c<a&&this.options.page!==c){var b={first:this.options.rows*c,rows:this.options.rows,page:c,pageCount:a,pageLinks:this.options.pageLinks};
this.options.page=c;
this._updateUI(b);
this._trigger("paginate",null,b)
}},_updateUI:function(b){for(var a in this.paginatorElements){ElementHandlers[a].update(this.paginatorElements[a],b)
